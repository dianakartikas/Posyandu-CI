...
<div class="card-body">
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            ...
            <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#formModalKader">
                <i class="fa fa-plus-circle"></i> Tambah Kader
            </button>
        </div>
                        ...
                        foreach ($users as $user) : ?>
                            <tr class="text-center">
                                ...
                                <td>
                                    ...
                                    <button class="btn btn-warning btn-sm" data-toggle="modal" data-target="#modaleditUser<?= $user->userid; ?>">
                                        <i class="fas fa-pencil-alt"></i>
                                    </button>
                                    <?php if ($user->name == 'user' || $user->name == 'kader') : ?>
                                        <button class="btn btn-danger btn-sm confirm_del_btn" value="<?= $user->userid; ?>"><i class="fas fa-trash"></i></button>
                                </td>
...
<?php foreach ($users as $user) : ?>
                ...
                <div class="modal-body">
                    <form action="/admin/save" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="fullname" id="fullname" class="form-control<?= ($validation->hasError('fullname')) ? ' is-invalid' : '' ?>" value="<?= old('fullname'); ?>">
                            <div class="invalid-feedback">
                                <?= $validation->getError('fullname'); ?>
                            </div>
                        </div>
    ...
    <div class="modal fade" id="formModalKader" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    <form action="/admin/savekader" method="post" enctype="multipart/form-data">
                        <?= csrf_field(); ?>
                        <div class="form-group">
                            <label for="username">Username</label>
                            <input type="text" name="username" id="username" class="form-control<?= ($validation->hasError('username')) ? ' is-invalid' : '' ?>" value="<?= old('username'); ?>">
    ...
    <!-- <script>
        ...
        //     gambarLabel.textContent = user_image.files[0].name;
...
<script>
    ...
    function previewImg() {
        ...
        const gambar = document.querySelectorAll("[id='user_image']");
        const gambarLabel = document.querySelector('.custom-file-label');
        const imgPreview = document.querySelector('.img-preview');
        for (const i = 0; i < gambar.length; i++)
            for (const i = 0; i < gambarLabel.length; i++)
                for (const i = 0; i < gambar.length; i++)
                    gambarLabel.textContent = gambar.files[i].name;
